...
<div class="w-full">
    ...
    <select 
        ...
        {{ $required ? 'required' : '' }}
        {{ $attributes->merge([
            'class' => 'w-full bg-ticketor-dark/80 border ' . ($errors->has($name) ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-gray-800 focus:border-ticketor-neon focus:ring-ticketor-neon') . ' rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:ring-1 transition'
        ]) }}
    >
        {{ $slot }}
    </select>
